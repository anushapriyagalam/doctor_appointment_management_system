...
const DoctorAppointments = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8300/appointment/doctorName/${doctorName}`)
      .then(response => {
        ...
        if (Array.isArray(data)) {
          ...
          const now = new Date();
          const updatedRatingEligibility = data.reduce((acc, app) => {
            const selectedDateTime = new Date(`${app.selectedDate} ${app.selectedTime}`);
            const hasRating = ratings.some(rating => rating.appointmentId === app.appointmentId);
            acc[app.appointmentId] = !hasRating && (new Date() - selectedDateTime >= 30 * 60 * 1000) && app.status === 'BOOKED';
            return acc;
          }, {});
